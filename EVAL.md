...
## Frozen evidence-group policy (before validation)
Added explicit missing-attribute guards, HDG and SS-grade handling, SDS expansion, and a conservative complex-request flag. An omitted critical attribute can be supplied by a unique issue-free identifier, but contradictions remain blockers. This is deliberately conservative: unknown vocabulary, bolt/thread dimensions, negation semantics and numeric pack-count parsing are not fully covered.

`python3 fit_policy.py` fits the immutable development partition only. Confidence is the Beta(1,1) posterior mean of top-candidate correctness in six fixed evidence groups (no candidate gets zero), not raw similarity. Groups are identifier, unique normalised exact text, other lexical score >=0.90, remaining lexical, blocked, and no candidate. Issue-bearing rows cannot auto-match. Enable a group only with >=10 examples, >=98% observed correctness and posterior mean >760/820. Wilson intervals are reported, not misrepresented as guarantees. These gates and the resulting policy.json are frozen before validation.

Development group counts: identifier 15/15 correct, lexical_high 33/33, lexical_other 56/56, lexical_exact 55/70, blocked 41/95, no_candidate 0/37. Enabled groups are identifier, lexical_high and lexical_other. Their posterior means are 0.9412, 0.9714 and 0.9828 respectively. This non-monotonic relation to similarity reflects supplied labels, not a general claim that typos make a request safer. The design may capture dataset-specific annotation patterns; held-out validation is essential and small groups have wide uncertainty. Original labels remain unchanged. This is coarse empirical calibration, not a guarantee on individual rows or new tenants.
